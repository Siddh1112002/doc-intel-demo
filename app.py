...
# Allow frontend on Vercel + (optionally) local dev
origins = [
    "https://doc-intel-demo.vercel.app",
    # uncomment these if you want local testing from a file or localhost
    # "http://localhost:5173",
    # "http://localhost:3000",
    # "http://127.0.0.1:8000",
]
...
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],        # or ["*"] while testing
...
def run_ocr_on_file(path: str) -> Dict[str, Any]:
    ...
    try:
        ...
        return {
            "error": "ocr_failed",
            "detail": f"fallback_failed: {type(e).__name__}: {e}",
            "trace": traceback.format_exc(),
        }
...
async def upload_doc(file: UploadFile = File(...)):
    ...
    try:
        ...
        return JSONResponse(
            status_code=500,
            content={"filename": filename, "text": "", "error": f"save_failed: {e}"},
        )
...
def export_pdf(filename: str = Query(..., description="Name of uploaded file (e.g. sample_invoice.pdf)")):
    ...
    if FPDF is None:
        return JSONResponse(
            status_code=500,
            content={"error": "fpdf_missing", "detail": "install fpdf to generate PDF: pip install fpdf"},
        )
    ...
    try:
        ...
        bio.seek(0)
        headers = {
            "Content-Disposition": f'attachment; filename="{os.path.splitext(filename)[0]}_summary.pdf"',
        }
...
async def save_corrections(payload: Dict[str, Any]):
    ...
    if not filename or fields is None:
        return JSONResponse(
            status_code=400,
            content={"error": "bad_request", "detail": "filename and fields required"},
        )
    path = os.path.join(UPLOAD_DIR, f"{filename}.json")
    out = {}
...
def health():
    return {
        "status": "ok",
        "ocr_module": bool(ocr_extract),
        "summarize_module": bool(summarize),
        "parser_rules": bool(parser_rules),
    }
